...
def write_ts_properties_to_csv(ts_properties_dict : dict, split : str) -> None:
    ...
    """
    dict_path = f"data/pre-training/{dataset}/{split}.csv"
...
def combine_HDFs(paths : list, training_set : bool, split : str) -> None:
    ...
    new_hdf_file = h5py.File(f"data/pre-training/{dataset}/{split}.h5", "a")
...
if __name__ == "__main__":
    ...
    if args.type == "split":
        ...
        max_n_nodes      = params["max_n_nodes"]    # maximum number of nodes in the data

        # # 1) combine the training files
        # path_list        = [f"data/pre-training/{dataset}_{i}/train.h5" for i in range(0, n_training_splits)]
        # combine_HDFs(path_list, training_set=False, split="train")

        # 2) combine the test files
        path_list        = [f"data/pre-training/{dataset}_{i}/test.h5" for i in range(0, n_test_splits)]
        combine_HDFs(path_list, training_set=False, split="test")

        # 3) combine the validation files
        path_list        = [f"data/pre-training/{dataset}_{i}/valid.h5" for i in range(0, n_valid_splits)]
