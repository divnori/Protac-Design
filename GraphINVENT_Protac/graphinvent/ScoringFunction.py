...
from ProtacScoringFunction import *
...
class ScoringFunction:
    ...
    def compute_score(self, graphs : list, termination : torch.Tensor,
        ...
        print(f"contributions_to_score {contributions_to_score}")
        ...
        if len(self.score_components) == 1:
            ...
            print('in continuous')
            final_score = contributions_to_score[0]
            print(f"structure score tensor {contributions_to_score[1]}")
        ...
        print(f"final score before {final_score}")
        ...
        print(f"termination score {termination}")

        print(f"final score after {final_score}")
    ...
    def get_contributions_to_score(self, graphs : list) -> list:
        ...
        for score_component in self.score_components:
            if "target_size" in score_component:
            ...
            elif "protac_activity" in score_component:
                mols = [graph.molecule for graph in graphs]
                ...
                score = self.compute_protac_activity(mols)

                print(f"activity scores {score}")

                contributions_to_score.append(score)

            elif "structure" in score_component:
                mols = [graph.molecule for graph in graphs]
                score = self.compute_structure(mols)
                print(f"structure score {score}")
                contributions_to_score.append(score)

            # elif "solubility" in score_component:
            #     mols = [graph.molecule for graph in graphs]
            #     score = self.compute_solubility(mols)
            #     print(f"structure score {score}")
            #     contributions_to_score.append(score)
    ...
    def compute_protac_activity(self, mols : list) -> list:
        """
        Args:
        ----
        ...
        Returns:
        -------
            activity (list) : Contains predicted protac activity for input molecules.
        """
        ...
        for idx, mol in enumerate(mols):
            score = predictProteinDegradation(mol, self.constants)
            activity[idx] = score

        print(f"shape of activity tensor is {activity.shape}")
        print(f"sum of activity tensor is {sum(activity.tolist())}")

        return activity

    def compute_structure(self, mols : list) -> list:
        """
        Args:
        ----
            mols (list) : Contains `rdkit.Mol` objects corresponding to molecular
        ...
        Returns:
        -------
            activity (list) : Contains predicted structures for input molecules (0 for non-protac, 1 for protac).
        """
        ...
        for idx, mol in enumerate(mols):
            score = predictStructure(mol, self.constants)
            activity[idx] = score
