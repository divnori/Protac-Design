...
class Workflow:
    ...
    def preprocess_train_data(self) -> None:
        ...
        print("before initializing data processor", flush = True)
        train_set_preprocesser = DataProcesser(path=self.constants.training_set,
                                               is_training_set=True)
        print("initialized data processor", flush=True)
    ...
    def define_model_and_optimizer(self) -> Tuple[int, int]:
        ...
        if self.constants.job_type == "fine-tune":
            ...
            print(f"epochs is {self.constants.epochs}", flush=True)
            print(f"max lr is {self.constants.max_rel_lr}", flush=True)
            print(f"min lr is {self.constants.min_rel_lr}", flush=True)
    ...
    def preprocess_phase(self) -> None:
        ...
        if not self.constants.restart:
            ...
            if os.path.exists(self.train_smi_path):
                print("train smi path exists",flush=True)       
                self.preprocess_train_data()
            print("done with all conditionals",flush=True)

        else:  # restart existing preprocessing job
            print("entered restart conditional",flush=True)
